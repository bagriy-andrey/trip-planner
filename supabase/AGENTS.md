...
- SPEC-06 / PLAN-06 (local only): `migrations/20260924120000_profiles.sql` — `profiles` (one row per `auth.users`, cascade, RLS, no client `delete`); pgTAP `tests/profiles_rls.test.sql` (21) + `tests/profiles_constraints.test.sql` (17).
