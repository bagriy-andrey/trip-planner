...
- `src/hotels/`: the `trip_hotels` form schema (`schemas.ts`, stable error ids in `errorCodes.ts`, lengths in code points, cost pair, 365-night cap), `countNights` (calendar dates in the hotel's zone, DST-safe, `nights.ts`), `breakfastDaysRange` (`breakfast.ts`), `parseMapsUrl` (`mapsUrl.ts`, https + Google Maps host whitelist, hand-rolled ASCII parsing), row/write mappers (`rows.ts`). UTC instants reuse `segments/time.ts`.
