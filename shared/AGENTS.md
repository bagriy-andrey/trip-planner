...
## Status
`@tripplanner/shared` exists (SPEC-02 / PLAN-02, extended by SPEC-03 / PLAN-03 and SPEC-04 / PLAN-04): `package.json` exports TS source directly (no build step), `tsc --noEmit` + vitest (`pnpm --filter @tripplanner/shared test`, 713 tests in 41 files).
...
- `src/profile/` (SPEC-06): profile row schema (format only, unknown directory values accepted), write validation against the directories, the city/country/home-airport link rules (`applyProfileChoice`; `applyCustomCity` for an own city not in the directory, with `normalizeCityName`/`isValidCityName`), `homeCurrencyDefault`, stable error ids. The first-segment prefill (`firstSegmentPrefill`) lives in `src/segments/prefill.ts`; the hotel form's currency-without-amount rule is in `src/hotels/`.
...
- `src/cars/` (SPEC-07): the `trip_cars` form schema, stable error ids, row/write mappers and `carReturnAfterFlight` (soonest flight departing after pickup).
- `src/db/database.types.ts`: generated Supabase types (`public.trips`, `public.trip_segments`, `public.trip_hotels`, `public.trip_cars`), exported type-only from `src/index.ts`; regenerate after each migration.
