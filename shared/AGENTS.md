...
## Status
`@tripplanner/shared` exists (SPEC-02 / PLAN-02, extended by SPEC-03 / PLAN-03 and SPEC-04 / PLAN-04): `package.json` exports TS source directly (no build step), `tsc --noEmit` + vitest (`pnpm --filter @tripplanner/shared test`, 485 tests in 19 files).
...
- `src/db/database.types.ts`: generated Supabase types (`public.trips`, `public.trip_segments`), exported type-only from `src/index.ts`; regenerate after each migration.
