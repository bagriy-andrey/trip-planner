...
## Status
`@tripplanner/shared` exists (SPEC-02 / PLAN-02, extended by SPEC-03 / PLAN-03 and SPEC-04 / PLAN-04): `package.json` exports TS source directly (no build step), `tsc --noEmit` + vitest (`pnpm --filter @tripplanner/shared test`, 648 tests in 36 files).
...
- `src/profile/` (SPEC-06): profile row schema (format only, unknown directory values accepted), write validation against the directories, the city/country/home-airport link rules, `homeCurrencyDefault`, stable error ids. The first-segment prefill (`firstSegmentPrefill`) lives in `src/segments/prefill.ts`; the hotel form's currency-without-amount rule is in `src/hotels/`.
