...
- Works: `pnpm -r typecheck`, `pnpm -r test` (jest-expo, 919 tests in 57 suites, ~30 s, incl. guardrail tests: route contract, static rules, `backend-only-behind-the-boundary`: `supabase-js`/network only inside `src/lib/supabase`), `npx expo config --type public`. Gotchas: `insights.md`.
