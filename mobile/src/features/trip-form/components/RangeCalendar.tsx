...
/** A month grid in the rental-dates style (day circles, one band for the range), weeks from Monday. It only reports taps; what a tap means is `pickRangeDate` in `shared`. */
export function RangeCalendar({ start, end, initialMonthOf, onPick, testID }: RangeCalendarProps) {
            ...
            return (
              ...
              >
                <View style={[styles.circle, selected && { backgroundColor: tokens.accent }]}>
                  <AppText variant="mono" color={selected ? "onAccent" : "text"}>
                    {Number(day.slice(8))}
                  </AppText>
                </View>
              </Pressable>
