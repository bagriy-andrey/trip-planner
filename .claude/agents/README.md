...
(`mobile/`, later `web/`) + Supabase backend (`supabase/`: Postgres, Auth, Storage, Edge Functions; ADR-001) + shared Zod contracts/domain logic (`shared/`) + Maestro e2e (`e2e/`).
...
| [architecture-reviewer](architecture-reviewer.md) | Read-only. Supabase RLS/migrations/secrets, mobile placement + iOS/Android parity, shared-logic single source of truth, package boundaries. CRITICAL/WARNING/INFO + BLOCKED/PASS. | `Read, Grep, Glob, Bash` (`disallowedTools: Write, Edit`) |
...
way around — e.g. `implementation-planner` and `implementer` both apply `supabase-backend`,
...
- Package set is `mobile/ supabase/ shared/ e2e/` (backend switched from Fastify to Supabase on 2026-09-18, see ADR-001); skill-routing tables in `implementation-planner`,
