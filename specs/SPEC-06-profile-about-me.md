...
- Свободный ввод АЭРОПОРТА, страны или валюты вне справочника, пополнение справочников и удаление
  синтетических аэропортов из `airports.ts` (отдельная задача данных, см. Q6). Свой ГОРОД вводить
  можно (поправка 2026-09-25, ниже).
...
| `home_city_name` | Свой город, если его нет в справочнике: текст пользователя, 2–80 символов, без пробелов по краям и без переводов строки. Поправка 2026-09-25 | nullable |
...
**Поправка 2026-09-25: свой город.** Список городов в справочнике ограничен, поэтому в шторке города можно
ввести своё название (строка «Добавить «текст»» первой при непустом запросе, если такого города нет в
списке). `home_city_place_id` и `home_city_name` взаимоисключающие (CHECK `profiles_home_city_exclusive`):
выбор города из списка очищает своё название и наоборот. Свой город НЕ подставляет страну проживания и
домашний аэропорт и не сверяется со страной. «Не указано» очищает оба поля. Текст нормализуется (пробелы
схлопываются, края обрезаются) в `shared/` (`normalizeCityName`); границы 2–80 символов в CHECK
`profiles_home_city_name_fmt` и в `shared/` (`isValidCityName`). Значение показывается как введено, без флага.
...
- **AC-18.** IF по запросу ничего не найдено, THEN the system **shall** показать «Ничего не нашли»
  ...
  принимается: значение выбирается только из справочника. ИСКЛЮЧЕНИЕ (поправка 2026-09-25): в шторке
  города при непустом запросе всегда есть строка «Добавить «текст»», поэтому «ничего не нашли» там
  не бывает. WHERE поле в текущем контексте
...
## Untrusted inputs
- Значения профиля выбираются из встроенных справочников. Единственный свободный текст — свой город
  (`home_city_name`, поправка 2026-09-25): он нормализуется, ограничен 80 символами, только показывается
  и нигде не интерпретируется; кроме него — поисковый запрос. Поисковый запрос — только данные для сравнения префиксов:
